app.controller('quizCtrl', ['$scope', '$http', 'helperService','myService','$timeout','$interval', function ($scope, $http, helper,myService,$timeout,$interval) {
    ...
    $scope.data = "test(4).json"; //quiz json file.
...
$scope.questions[$scope.currentPage-1].object.options[id-1].Selected=true;
...
for( var i=0; i<$scope.questions[id-1].object.options.length;i++){
    temp[i]=$scope.swapOptions[i];

}
var temp2=temp.join('');
$scope.selectAns($scope.currentPage,temp2); 
